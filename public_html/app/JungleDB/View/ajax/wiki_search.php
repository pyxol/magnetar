<?php
	...
	$sph_result = $cl->Query($query, "index_wikipedia");
	...
	if(!empty($sph_result['matches'])) {
		$found_ids = array_map(function($match) {
			return $match['attrs']['wiki_id'];
		}, $sph_result['matches']);
		
		$db->select_db(WIKIPEDIA_DATABASE);
		$results = $db->get_results("SELECT `wiki_id`, `title` FROM `articles` WHERE `wiki_id` IN ('". implode("','", $found_ids) ."') ORDER BY FIELD(`wiki_id`, '". implode("','", $found_ids) ."')");
	}
